...
import torch
from models.model import generate_model, generate_fa_model
...
def test():
    ...
    model0, parameters0 = generate_fa_model(opt)
    ...
    #pdb.set_trace()
    if opt.resume_path0:
        print('loading checkpoint {}'.format(opt.resume_path0))
        checkpoint = torch.load(opt.resume_path0)
    ...
    with torch.no_grad():
        ...
        for i, (clip, label) in enumerate(val_dataloader):
            ...
            if opt.only_RGB: # 肯定不走这路
                ...
                RGB_clip  = clip[0:3,:,:,:]
                Flow_clip = clip[3:,:,:,:]
                inputs0 = torch.Tensor(int(MARS_clip.shape[1]/opt.sample_duration), 3, opt.sample_duration, opt.sample_size, opt.sample_size)
                ...
                for k in range(inputs1.shape[0]):
                    inputs0[k,:,:,:,:] = MARS_clip[:,k*opt.sample_duration:(k+1)*opt.sample_duration,:,:]
                ...
                inputs_var0 = Variable(inputs0)
                inputs_var1 = Variable(inputs1)
                inputs_var2 = Variable(inputs2)   
            
            outputs_var0= model0(inputs_var0)
            outputs_var1= model1(inputs_var1)
            outputs_var2= model2(inputs_var2)
            #pdb.set_trace()
            outputs_var = torch.mean(torch.cat((outputs_var0, outputs_var1, outputs_var2), dim=0), dim=0).unsqueeze(0)
