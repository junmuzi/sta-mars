  ...
  for ic,cls in enumerate(class_list):
    ...
    for v in tqdm(vlist):
        ...
        try:  
            ...
            # extract flows 
            #os.system('export CUDA_VISIBLE_DEVICES=%d; /data1/lijun/sta/codes/MARS/utils1/tvl1_videoframes "%s" %d "%s" '%(gpu, os.path.join(outdir, '%05d.jpg'), nframes, os.path.join(outdir,'%05d_flow256crop.flo') ))
            #print('export CUDA_VISIBLE_DEVICES=%d; /data1/lijun/sta/codes/MARS/utils1/tvl1_videoframes "%s" %d "%s" '%(gpu, os.path.join(outdir, '%05d.jpg'), nframes, os.path.join(outdir,'%05d_flow256crop.flo') ))
