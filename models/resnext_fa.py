...
class ResNeXt_fa(nn.Module):
    ...
    def __init__(self,
        ...
        self.layer2 = self._make_layer(
            block, 256, layers[1], shortcut_type, cardinality, stride=2, img_size = 14)
        self.layer3 = self._make_layer(
            block, 512, layers[2], shortcut_type, cardinality, stride=2, img_size = 7)
        self.layer4 = self._make_layer(
            block_fa, 1024, layers[3], shortcut_type, cardinality, stride=2, img_size = 4)
        last_duration = int(math.ceil(sample_duration / 16))
        last_size = int(math.ceil(sample_size / 32))
        self.avgpool = nn.AvgPool3d(
            (last_duration, last_size, last_size), stride=1)
